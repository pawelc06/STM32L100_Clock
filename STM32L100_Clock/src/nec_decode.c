...
void NEC_StartTimer() { // Timer for overflow detection
	...
	TIM_TimeBaseStructure.TIM_Prescaler = 32 - 1; // 32 MHz Clock down to 1 MHz (adjust per your clock to count microseconds)
...
void NEC_StopTimer() {
	...
	/* Time base configuration */
	TIM_TimeBaseStructure.TIM_Period = 12000; // 12ms oveflow
	TIM_TimeBaseStructure.TIM_Prescaler = 16 - 1; // 42 MHz Clock down to 1 MHz (adjust per your clock to count microseconds)
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
	TIM_TimeBaseInit(TIM3, &TIM_TimeBaseStructure);
	/* TIM IT enable */
